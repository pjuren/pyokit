...
class InterfaceException(Exception):
  ...
  """Exception class for errors that might occur when parsing command lines."""
...
class Option(object):
  ...
  def __init__(self, short, long, description, argName=None,
               ...
               special=False):
    """Constructor for Option class. See class docstring for param defs."""
    self.short = short
    self.long = long
    ...
    if self.type != DEFAULT_TYPE and argName is None:
...
class CLI(object):
  ...
  def __init__(self, progName, shortDesc, longDesc):
    ...
    Builds a CLI object that defines an interface, but has not yet been
    populated with user-supplied data; see class-level documentation for
